...
struct object *screenfetch(struct objectArray *args)
{
    ...
    for (int i = strlen(fetch_logo[0]); i < 25; i++)
        putchar(' ');
    print_color("[ZHIR OS]\n", 0b1110);
    ...
    for (int i = strlen(fetch_logo[1]); i < 25; i++)
        putchar(' ');
    ...
    for (int i = strlen(fetch_logo[2]); i < 25; i++)
        putchar(' ');
    ...
    for (int i = strlen(fetch_logo[3]); i < 25; i++)
        putchar(' ');
    ...
    for (int i = strlen(fetch_logo[4]); i < 25; i++)
        putchar(' ');
    ...
    for (int i = strlen(fetch_logo[5]); i < 25; i++)
        putchar(' ');
    print("\n");
    return 0;
}
...
struct object *write(struct objectArray *args)
{
    if (args->count < 3)
    {
        KLOGE("use write <diskletter <file name> <data>");
        return 0;
    }
    int diskid = *(char *)args->objs[0].data - 'a';
    struct disk *dsk = disks[diskid];
    BPB *bpb = read_first_sector(dsk);
    RootDir *rootdir = calculateRootDir(bpb);
    u8 *root_buf = readRootDir(dsk, rootdir, bpb);
    DirEntry *entry = find_file(root_buf, bpb, args->objs[2].data);

    if (!entry)
    {
        entry = create_file(root_buf, bpb, args->objs[2].data);

        if (!entry)
        {
            KLOGE("no that file and can't create of it");
            free(bpb);
            free(root_buf);
            free(rootdir);
